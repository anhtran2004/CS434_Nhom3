...
namespace Webebook.WebForm.User
{
    public partial class thanhtoan : System.Web.UI.Page
    {
        ...
        protected void btnXacNhan_Click(object sender, EventArgs e)
        {
            ...
            {
                string ownedBooksList = string.Join(", ", alreadyOwnedBooks.Select(b => $"'{HttpUtility.HtmlEncode(b)}'"));
                string confirmMessageBase = $"Bạn đã sở hữu sách sau: {ownedBooksList}.\\n\\nChọn OK để tới Tủ sách, Cancel để quay lại Giỏ hàng.";
                string tusachUrl = ResolveUrl("~/WebForm/User/tusach.aspx");
                string giohangUrl = ResolveUrl("~/WebForm/User/giohang_user.aspx");
                var serializer = new JavaScriptSerializer();
                string encodedMessage = serializer.Serialize(confirmMessageBase);
                string script = $@"
                    var overlay = document.getElementById('loadingOverlay');
                    if (overlay) overlay.classList.remove('visible');
                    if (confirm({encodedMessage})) {{
                        window.location.href='{tusachUrl}';
                    }} else {{
                         var btn = document.getElementById('{btnXacNhan.ClientID}');
                         if(btn) btn.removeAttribute('disabled');
                         window.location.href='{giohangUrl}';
                    }}";
                ScriptManager.RegisterStartupScript(this, GetType(), "AlreadyOwnedConfirmRedirect", script, true);
